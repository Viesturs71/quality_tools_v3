...
# =============================
# SUPER CLEANER PRO versija (ar tqdm progress bar)
# =============================
...
FOLDER_SIMILARITY_THRESHOLD = 0.8
...
def delete_files(files):
    for file in tqdm(files, desc="Dzēšam failus"):
...
def load_project_sources():
    ...
    for filepath in tqdm(all_files, desc="Ielādējam failus"):
        try:
...
def find_unused_folders(project_sources):
    ...
    used_folders = set()
    for folder in tqdm(all_folders, desc="Pārbaudām folderus"):
...
if __name__ == '__main__':
    ...
    if unused_folders:
        for folder in tqdm(unused_folders, desc="Dzēšam unused folders"):
            try:
